#!/bin/bash
# Train the segmentation model (PaliGemma-2 + frozen FT SigLIP, report + VQA + segmentation).
#
# For SLURM clusters, submit with e.g.:
#   sbatch --gres=gpu:1 --mem=80G -p gpu run_train_segment.sh
# or run directly on a machine with a GPU:
#   bash run_train_segment.sh
#
...
CONFIG_PATH="configs/radgrounder_segmentation.json"

python train_groundedgemma.py --config "${CONFIG_PATH}"
echo "job finished."
