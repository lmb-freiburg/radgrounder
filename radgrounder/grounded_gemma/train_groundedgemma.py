...
def main():
    parser = argparse.ArgumentParser(description="Train GroundedGemma for medical segmentation")
    ...
    parser.add_argument("--resume_from_checkpoint", type=str, default=None, 
                        help="Path to checkpoint to resume training from (e.g., './runs/model_name')")
    ...
    if not config.get("run_name"):
        ...
        run_name = f"{model_short}_segment_{notes}_bs_{batch_size}x{grad_accum}={effective_bs}_{config_hash}" if notes else f"{model_short}_segment_bs_{batch_size}x{grad_accum}={effective_bs}_{config_hash}"
    ...
    if resume_from_checkpoint and processor_path:
        ...
        elif processor_path.parent.parent.exists():
            # Try parent run directory
            run_processor_path = processor_path.parent.parent
            if any(run_processor_path.glob("*processor*")):
        ...
        # Add special tokens before loading model
        logger.info(f"Processor tokenizer vocab size: {len(processor.tokenizer)}")
        processor.tokenizer.add_special_tokens({'additional_special_tokens': [SEG_START, SEG_END]})
        end_special_token_id = processor.tokenizer.convert_tokens_to_ids(SEG_END)
    ...
    # Inspect segmentation head
    for name, param in model.segmentation_head.named_parameters():
        logger.info(f"{name}: min={param.min().item():.4f}, max={param.max().item():.4f}, mean={param.mean().item():.4f}, std={param.std().item():.4f}")
    
    model.set_seg_token_id(end_special_token_id)
    logger.info(f"Processor tokenizer vocab size after adding tokens: {len(processor.tokenizer)}")
    model.resize_token_embeddings(len(processor.tokenizer))

    # Set trainable layers
    train_encoder = config.get("train_encoder", False)
    train_projector = config.get("train_projector", False)
    train_lang_model = config.get("train_lang_model", True)
    train_seg_head = config.get("train_seg_head", True)
    set_trainable_layers(model, train_encoder, train_projector, train_lang_model, train_seg_head)
    ...
    val_dataset_manager = DatasetManager(
        dataset_name=config.get("dataset_name", "refrad2d_segment_merged"),
        split="val",
        img_size=img_size,
        eval_mode=False,
        language=language,
        ...
        only_segmented=config.get("only_segmented", False),
    ...
    if EVAL_STEPS < GRAD_ACCUM:
        EVAL_STEPS = GRAD_ACCUM

    if SAVE_STEPS < GRAD_ACCUM:
        SAVE_STEPS = GRAD_ACCUM
    ...
    trainer_config.update({
        ...
        "wandb_project": config.get("wandb_project", "radgrounder-groundedgemma"),
    })

    model = train_with_ggemma_trainer(model, train_dataset, val_dataset, processor, trainer_config)
    ...
    # ----- Start: Added code for validation inference -----

    # print("Training complete. Running inference on validation set with the best model...")
    # del model
    ...
    # os.system(f"python eval_refrad2d_batch.py --model_path {output_dir} --dataset_path {val_path} --batch_size {BATCH_SIZE_STEP}\
    #         --img_size {args.img_size} --language {args.language} --torch_dtype {TORCH_DTYPE} --seq_len {SEQLEN}\
    #         --model_id {MODEL_ID} --dataset_size 5000 --prefix_style both --body_part {args.body_part}")

def train_with_ggemma_trainer(model, train_dataset, val_dataset, processor, config):
    from ggemma_trainer import GroundedGemmaSFTTrainer
    from trl import SFTConfig
    
    logger.info("Starting GroundedGemma SFT training...")
    
    use_wandb = config.get("use_wandb", False)
    if use_wandb:
        import wandb
        wandb.init(
            project=config.get("wandb_project", "radgrounder-groundedgemma"),
    ...
    # Create the GroundedGemma SFTTrainer
    trainer = GroundedGemmaSFTTrainer(
        model=model,
...
def train_with_sft_trainer(model, train_dataset, val_dataset, processor, args):
    from trl import SFTTrainer, SFTConfig
    from peft import LoraConfig
    # medgemma lora config
    ...
    sft_config = SFTConfig(
        ...
        run_name=args.run_name,                                 # Name of the run for logging
        num_train_epochs=args.epochs,                           # Number of training epochs
        per_device_train_batch_size=args.batch_size_step,       # Batch size per device during training
        per_device_eval_batch_size=args.batch_size_step,              # Batch size per device during evaluation
        gradient_accumulation_steps=args.grad_accum,             # Number of steps before performing a backward/update pass
        max_length=args.seq_len,                                       # Maximum sequence length for training
        gradient_checkpointing=True,                             # Enable gradient checkpointing to reduce memory usage
        optim="adafactor", #"adamw_torch_fused",                               # Use fused AdamW optimizer for better performance
        # optim="adamw_torch_fused",                               # Use fused AdamW optimizer for better performance
        logging_strategy="steps",
        logging_steps=args.save_steps,                                        # Number of steps between logs
        save_strategy="steps",                                   # Save checkpoint every epoch
        save_steps=args.save_steps,                                   # Number of steps between saves
        save_total_limit=args.save_limit,                                     # Limit the total number of checkpoints saved
        save_safetensors=False,
        eval_strategy="steps",                                   # Evaluate every `eval_steps`
        eval_steps=args.save_steps,                                           # Number of steps between evaluations
        ...
        report_to=[],                                            # wandb disabled by default (unused path)
    ...
    trainer = SFTTrainer(
        ...
        data_collator=get_collate_fn(processor, args.seq_len, args.torch_dtype),
        processing_class=processor,
        args=sft_config,
    )
...
def set_trainable_layers(model, train_encoder, train_projector, train_lang_model=True, train_seg_head=True):
    ...
    for param in model.language_model.parameters():
        param.requires_grad = train_lang_model

    for param in model.segmentation_head.parameters():
        param.requires_grad = train_seg_head
    ...
    logger.info(f"  - Language model: {train_lang_model}")
    logger.info(f"  - Segmentation head: {train_seg_head}")
